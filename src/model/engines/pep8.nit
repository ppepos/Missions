...
"""
	...
	redef fun clean_artifacts(program) do
		var ws = program.workspace
		if ws == null then return
		var pepo = (ws / "source.pepo").to_path
		if pepo.exists then pepo.delete
		var err = pepo.last_error
		if err != null then print "Error when cleaning artifacts for {program.mission}: {err}"
	end
end
